"""Train SNN on Poker-DVS benchmark.

Architecture: 2450 -> hidden (recurrent adLIF) -> 4
4-class poker card suit classification from DVS recordings.

Usage:
    python poker_dvs/train.py --epochs 100 --device cuda:0
"""
...
from poker_dvs.loader import PokerDVSDataset, collate_fn, N_CHANNELS, N_CLASSES


class PokerDVSSNN(nn.Module):
    """Recurrent SNN for Poker-DVS card classification."""
...
def main():
    parser = argparse.ArgumentParser(description="Train SNN on Poker-DVS")
    parser.add_argument("--data-dir", default="data/poker_dvs")
    ...
    parser.add_argument("--save", default="poker_dvs_model.pt")
    ...
    print("Loading Poker-DVS dataset...")
    train_ds = PokerDVSDataset(args.data_dir, train=True, n_time_bins=args.time_bins)
    test_ds = PokerDVSDataset(args.data_dir, train=False, n_time_bins=args.time_bins)
    ...
    config = {
        ...
        'benchmark': 'poker_dvs', 'augment_fn': augment_fn,
