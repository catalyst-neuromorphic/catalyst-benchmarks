"""Train SNN on N-TIDIGITS18 benchmark.

Architecture: 64 -> hidden (recurrent adLIF) -> 18
18-class spike-encoded spoken digit classification.

Usage:
    python ntidigits/train.py --epochs 100 --device cuda:0
"""
...
from ntidigits.loader import NTIDIGITS18Dataset, collate_fn, N_CHANNELS, N_CLASSES


class NTIDIGITSSNN(nn.Module):
    """Recurrent SNN for N-TIDIGITS18 spoken digit classification."""
...
def main():
    parser = argparse.ArgumentParser(description="Train SNN on N-TIDIGITS18")
    parser.add_argument("--data-dir", default="data/ntidigits")
    ...
    parser.add_argument("--save", default="ntidigits_model.pt")
    ...
    print("Loading N-TIDIGITS18 dataset...")
    train_ds = NTIDIGITS18Dataset(args.data_dir, train=True,
                                   n_time_bins=args.time_bins)
    test_ds = NTIDIGITS18Dataset(args.data_dir, train=False,
                                  n_time_bins=args.time_bins)
    ...
    config = {
        ...
        'benchmark': 'ntidigits', 'augment_fn': augment_fn,
