"""Train SNN on Sequential CIFAR-10 (sCIFAR-10).

Architecture: 3 -> hidden (recurrent adLIF) -> 10
Tests very long-range temporal dependency (1024 timesteps).

Usage:
    python scifar10/train.py --epochs 100 --device cuda:0
"""
...
from scifar10.loader import SCIFAR10Dataset, collate_fn, N_CHANNELS, N_CLASSES


class SCIFAR10SNN(nn.Module):
    """Recurrent SNN for Sequential CIFAR-10."""
...
def main():
    parser = argparse.ArgumentParser(description="Train SNN on sCIFAR-10")
    ...
    parser.add_argument("--save", default="scifar10_model.pt")
    ...
    print("Loading Sequential CIFAR-10 dataset...")
    train_ds = SCIFAR10Dataset(train=True)
    test_ds = SCIFAR10Dataset(train=False)
    ...
    print(f"Train: {len(train_ds)}, Test: {len(test_ds)}, Seq len: 1024")
    ...
    config = {
        ...
        'benchmark': 'scifar10', 'augment_fn': None,
