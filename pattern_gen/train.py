"""Train SNN on Spike Pattern Generation benchmark.

Architecture: 50 -> hidden (recurrent adLIF) -> 8
8-class spatiotemporal spike pattern classification.
Self-generating data — no download needed.

Usage:
    python pattern_gen/train.py --epochs 100 --device cuda:0
"""
...
from pattern_gen.loader import PatternGenDataset, collate_fn, N_CHANNELS, N_CLASSES


class PatternGenSNN(nn.Module):
    """Recurrent SNN for spike pattern classification."""
...
def main():
    parser = argparse.ArgumentParser(description="Train SNN on Pattern Gen")
    ...
    parser.add_argument("--save", default="pattern_gen_model.pt")
    ...
    print("Generating spike pattern dataset...")
    train_ds = PatternGenDataset(n_samples=5000, train=True)
    test_ds = PatternGenDataset(n_samples=1000, train=False)
    ...
    config = {
        ...
        'benchmark': 'pattern_gen', 'augment_fn': None,
        'label_smoothing': 0.0,
        'model_config': {
