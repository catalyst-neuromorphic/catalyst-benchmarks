"""Train SNN on MIT-BIH Arrhythmia benchmark.

Architecture: 1 -> hidden (recurrent adLIF) -> 5
Real-world ECG heartbeat classification (5 AAMI superclasses).

Usage:
    python mitbih/train.py --epochs 100 --device cuda:0
"""
...
from mitbih.loader import MITBIHDataset, collate_fn, N_CHANNELS, N_CLASSES


class MITBIHSNN(nn.Module):
    """Recurrent SNN for MIT-BIH Arrhythmia classification."""
...
def main():
    parser = argparse.ArgumentParser(description="Train SNN on MIT-BIH")
    ...
    parser.add_argument("--save", default="mitbih_model.pt")
    ...
    train_ds = MITBIHDataset(train=True)
    test_ds = MITBIHDataset(train=False)
    ...
    config = {
        ...
        'benchmark': 'mitbih', 'augment_fn': None,
        'label_smoothing': 0.0,
        'model_config': {
