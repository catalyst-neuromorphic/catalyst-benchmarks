"""Train SNN on Sequential MNIST (S-MNIST) benchmark.

Architecture: 1 -> hidden (recurrent adLIF) -> 10
Pixel-by-pixel MNIST — tests long-term temporal memory (784 steps).

Usage:
    python smnist/train.py --epochs 50 --device cuda:0
"""
...
from smnist.loader import SMNISTDataset, collate_fn, N_CHANNELS, N_CLASSES


class SMNISTSNN(nn.Module):
    """Recurrent SNN for Sequential MNIST classification."""
...
def main():
    parser = argparse.ArgumentParser(description="Train SNN on S-MNIST")
    parser.add_argument("--data-dir", default="data/smnist")
    ...
    parser.add_argument("--save", default="smnist_model.pt")
    ...
    train_ds = SMNISTDataset(args.data_dir, train=True)
    test_ds = SMNISTDataset(args.data_dir, train=False)
    ...
    print(f"Train: {len(train_ds)}, Test: {len(test_ds)}, Seq length: 784")
    ...
    print(f"Model: 1->{args.hidden}->10 "
    ...
    config = {
        ...
        'benchmark': 'smnist', 'augment_fn': None,
