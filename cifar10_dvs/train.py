"""Train SNN on CIFAR10-DVS benchmark.

Architecture: 2048 -> hidden1 (recurrent adLIF) -> hidden2 (adLIF) -> 10
Readout: time-averaged membrane potential -> softmax

Usage:
    python cifar10_dvs/train.py --epochs 300 --device cuda:0
    python cifar10_dvs/train.py --neuron adlif --hidden1 512 --hidden2 256 --device cuda:0
"""
...
from cifar10_dvs.loader import CIFAR10DVSDataset, collate_fn, N_CHANNELS, N_CLASSES


class CIFAR10DVSSNN(nn.Module):
    """Two-layer SNN for CIFAR10-DVS classification.

    2048 -> hidden1 (recurrent adLIF) -> hidden2 (adLIF) -> 10
    """
...
def main():
    parser = argparse.ArgumentParser(description="Train SNN on CIFAR10-DVS")
    parser.add_argument("--data-dir", default="data/cifar10_dvs")
    ...
    parser.add_argument("--save", default="cifar10dvs_model.pt")
    ...
    train_ds = CIFAR10DVSDataset(args.data_dir, train=True, n_time_bins=args.time_bins)
    test_ds = CIFAR10DVSDataset(args.data_dir, train=False, n_time_bins=args.time_bins)
    ...
    config = {
        ...
        'benchmark': 'cifar10_dvs', 'augment_fn': augment_fn,
