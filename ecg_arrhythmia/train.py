"""Train SNN on ECG Arrhythmia classification benchmark.

Architecture: 1 -> hidden (recurrent adLIF) -> 5
5-class synthetic ECG arrhythmia classification.

Usage:
    python ecg_arrhythmia/train.py --epochs 100 --device cuda:0
"""
...
from common.neurons_n3 import GatedLIFNeuron
from common.training import run_training

from ecg_arrhythmia.loader import ECGArrhythmiaDataset, collate_fn, N_CHANNELS, N_CLASSES


class ECGSNN(nn.Module):
    """Recurrent SNN for ECG arrhythmia classification."""
...
class GatedECGSNN(nn.Module):
    """Recurrent SNN with GatedLIF neurons for ECG arrhythmia.

    Uses GatedLIFNeuron to simulate N3 approximate computing:
    the gate can suppress far-from-threshold neurons to save power.
    Post-training, run_approximate_sweep() evaluates graceful degradation.
    """

    def __init__(self, n_input=N_CHANNELS, n_hidden=128, n_output=N_CLASSES,
                 beta_out=0.9, threshold=1.0, dropout=0.2):
        super().__init__()
        self.n_hidden = n_hidden
        self.n_output = n_output
        ...
        self.lif1 = GatedLIFNeuron(n_hidden, threshold=threshold)
        self.lif2 = LIFNeuron(n_output, beta_init=beta_out,
                               threshold=threshold, learn_beta=True)
        self.dropout = nn.Dropout(p=dropout)
...
def main():
    parser = argparse.ArgumentParser(description="Train SNN on ECG Arrhythmia")
    ...
    parser.add_argument("--save", default="ecg_model.pt")
    ...
    parser.add_argument("--gated", action="store_true", default=False,
                        help="Use GatedLIF neurons (N3 approximate computing)")
    ...
    train_ds = ECGArrhythmiaDataset(n_samples=4000, train=True)
    test_ds = ECGArrhythmiaDataset(n_samples=1000, train=False)
    ...
    if args.gated:
        ...
        print(f"Model: {N_CHANNELS}->{args.hidden}(GatedLIF)->{N_CLASSES} "
              f"(N3 GATED, recurrent=on, dropout={args.dropout})")
    else:
        model = ECGSNN(
    ...
    config = {
        ...
        'benchmark': 'ecg_arrhythmia', 'augment_fn': None,
        'label_smoothing': 0.0,
        'model_config': {
            'n_input': N_CHANNELS, 'hidden': args.hidden,
            'n_output': N_CLASSES, 'neuron_type': 'gated' if args.gated else args.neuron,
