"""Train SNN on N-Cars (Prophesee Automotive) benchmark.

Architecture: 1800 -> hidden (recurrent adLIF) -> 2
Binary classification: car vs background.

Usage:
    python ncars/train.py --epochs 100 --device cuda:0
"""
...
from ncars.loader import NCarsDataset, collate_fn, N_CHANNELS, N_CLASSES


class NCarsSNN(nn.Module):
    """Recurrent SNN for N-Cars binary classification."""
...
def main():
    parser = argparse.ArgumentParser(description="Train SNN on N-Cars")
    parser.add_argument("--data-dir", default="data/ncars")
    ...
    parser.add_argument("--save", default="ncars_model.pt")
    ...
    train_ds = NCarsDataset(args.data_dir, train=True, n_time_bins=args.time_bins)
    test_ds = NCarsDataset(args.data_dir, train=False, n_time_bins=args.time_bins)
    ...
    config = {
        ...
        'benchmark': 'ncars', 'augment_fn': augment_fn,
